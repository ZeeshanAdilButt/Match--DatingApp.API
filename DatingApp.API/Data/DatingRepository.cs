...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            var users = _context.Users.Include(p => p.Photos).Include(x => x.Likees).Include(x => x.Likers).AsQueryable();
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
            ...
            if (likers)
            {
                var t = user.Likers.Where(u => u.LikeeId == id).ToList();
                var x = user.Likers.Where(u => u.LikeeId == id).Select(i => i.LikerId);

                return x;
            }
            else
            {
                var tt = user.Likees.Where(u => u.LikerId == id).ToList();
        ...
        {
            var messages = _context.Messages.Include(u=> u.Recipient).ThenInclude(p=>p.Photos)
